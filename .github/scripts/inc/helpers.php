...
function basePath(string $path = ''): string
{
    ...
    return str_replace('\\', '/', $path ? "{$baseDir}/{$path}" : $baseDir);
}
    ...
    try {
        ...
        if (\function_exists('proc_open')) {
            $command = $isWindows
                ...
                : "rm -rf {$path}";
            $process = Process::fromShellCommandline($command, basePath(), null, null, 1800);
 ...
 * @return bool
 */
function mkdirp(string $dir): bool {
...
function finalMessage(int $all, int $success): int
{
    ...
    $versionStr = $all === 1 ? 'one Gutenberg version' : sprintf('%d Gutenberg versions', $all);

    if ($success === $all) {
        $message = sprintf("Done adding %s.", $versionStr);
        fwrite(STDOUT, "\n{$message}\n");
    ...
    $error = (($success === 0) || ($all === 1))
        ? sprintf("Failed adding %s.", $versionStr)
        : sprintf("Failed adding %d out of %s.", $all - $success, $versionStr);
    fwrite(STDERR, "\n{$error}\n");
